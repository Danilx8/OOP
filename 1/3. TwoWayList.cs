namespace OOP
{
    ...
    public abstract class NewLinkedList<T> : ParentList<T>
    {
    }

    public abstract class TwoWayList
    {
        public const int LEFT_NIL = 0; // Left() не вызывалась 
        public const int LEFT_OK = 1; // Left() отработала нормально
        public const int LEFT_ERR = 2; // курсор в начале
        public const int LEFT_EMPTY = 3; // список пуст
        
        // Предусловие: слева от узла есть элемент
        // Постусловие: курсор сдвигается влево
        public abstract void Left();
        
        public abstract int GetLeftStatus(); // LEFT_*
    }
}